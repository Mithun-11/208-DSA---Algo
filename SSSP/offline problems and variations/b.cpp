...
void solve()
{
    ...
    vector<vector<ll>>min_cost(n+1, vector<ll>(3, INF));
    min_cost[1][0]=0;
    ...
    while(!q.empty())
    {
        ...
        for (auto [v,c] : adj[curr.pos])
        {
            ...
            if(curr.used==0 && curr_cost+c/2<min_cost[v][1])
            {
                min_cost[v][1]=curr_cost+c/2;
                q.push({min_cost[v][1],v,1});
            }

           else if(curr.used==1 && curr_cost+c/2<min_cost[v][2])
            {
                min_cost[v][2]=curr_cost+c/2;
                q.push({min_cost[v][2],v,2});
            }

        }
    }

    cout<<min({min_cost[n][1],min_cost[n][0],min_cost[n][2]});
